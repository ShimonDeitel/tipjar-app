...
final class TipjarTests: XCTestCase {
    ...
    func testFreeLimitBlocksAddition() {
        store.isPro = false
        for i in 0..<(TipjarStore.freeLimit + 5) {
            store.add(title: "Filler \(i)", amount: 1.0)
        }
        XCTAssertEqual(store.entries.count, TipjarStore.freeLimit)
    }
    ...
    func testCanAddMoreWhenPro() {
        ...
        XCTAssertGreaterThan(store.entries.count, TipjarStore.freeLimit)
    }
    ...
    func testFreeLimitAboveSeedCount() {
        XCTAssertGreaterThan(TipjarStore.freeLimit, 3)
    }
}
